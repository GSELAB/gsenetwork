...
//	rlpxstream.appendRaw(toPublic(_k).asBytes()); // for mdc-based signature
    ...
    h256 sighash(crypto::sha3(rlpx)); // H(type||data)

    /* remark by Jorge
    Signature sig = crypto::sign(_k, sighash); // S(H(type||data))
    ...
    core::sha3(signedRLPx).ref().copyTo(rlpxHash);
    */
...
Public RLPXDatagramFace::authenticate(bytesConstRef _sig, bytesConstRef _rlp)
{
    ...
    // Modify by Jorge
    //return core::recover(sig, sha3(_rlp));
