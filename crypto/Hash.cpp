...
h256 sha256(bytesConstRef input) noexcept
{
    ...
    secp256k1_sha256_write(&ctx, input.data(), input.size());
    h256 hash;
...
h160 ripemd160(bytesConstRef input)
{
	...
	// initialize
	MDinit(buffer);
	byte const* message = input.data();
	uint32_t remaining = input.size();	// # of bytes not yet processed
	...
	for (; remaining >= 64; remaining -= 64)
	{
		...
		MDcompress(buffer, current);
	}
	// length mod 64 bytes left

	// finish:
	MDfinish(buffer, message, input.size(), 0);
