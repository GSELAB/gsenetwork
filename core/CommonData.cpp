...
bool core::isHex(string const& _s) noexcept
{
	auto it = _s.begin();
...
bytes core::fromHex(std::string const& _s, WhenError _throw)
{
...
bytes core::asNibbles(bytesConstRef const& _s)
{
	std::vector<uint8_t> ret;
	ret.reserve(_s.size() * 2);
	for (auto i: _s)
...
std::string core::toString(string32 const& _s)
{
	std::string ret;
